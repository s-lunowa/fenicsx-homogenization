...
radius = 0.5 * (0.1778 + 0.127)
mu = 1e-3 # dynamic viscosity
rel_tol = 5e-14
current_dir = os.path.dirname(os.path.abspath(__file__))
dir_name = os.path.join(current_dir, "darcy", f"vessel_{args.case}")
...
if DX_VERSION == "0.9.0":
    msh, cell_tags, facet_tags = io.gmshio.read_from_msh(os.path.join(current_dir, "geometry", "artificial_vessel", "domain_3d.msh"), MPI.COMM_WORLD)
else:
    msh, cell_tags, facet_tags, *_ = io.gmsh.read_from_msh(os.path.join(current_dir, "geometry", "artificial_vessel", "domain_3d.msh"), MPI.COMM_WORLD)
...
porosity_file = os.path.join(current_dir, "permeability", "data", "artificial_vessel", file_middle + ".vtk")
